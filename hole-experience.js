...
(() => {
  ...
  // Replace Leaflet rendering with the Google adapter while keeping the existing round/shot model.
  ensureMap=function(){
    ...
    if(!map||!map.__shotTrackGoogle){
      try{if(map?.remove)map.remove();}catch{}
      el.innerHTML='';
      map=window.shotTrackGoogle.create(el,c,p=>mapTap(p));
      if(map)map.__shotTrackGoogle=true;
    }else{map.setCenter(c);}
    renderMap();
  };

  renderMap=function(){
    if(!map||!round||!map.__shotTrackGoogle)return;
    const h=round.holes[round.currentHole];
    window.shotTrackGoogle.render(map,h,currentMapCenter());
    $('mapReadout').textContent=h.greenCenter?'Google satellite · ball to green':'Google satellite · drag and zoom anytime';
  };

  function frameHole(){if(!map||!round||!map.__shotTrackGoogle)return;const h=round.holes[round.currentHole],ball=h.shots.length?h.shots[h.shots.length-1].end:h.start||h.teeCenter;if(ball)window.shotTrackGoogle.frame(map,ball,h.greenCenter);}
  ...
  begin=async function(){await oldBegin();if(!round)return;const h=round.holes[round.currentHole],geo=await findHoleGeometry(round.courseCenter,round.currentHole);if(geo?.tee)h.teeCenter=geo.tee;if(geo?.green)h.greenCenter=geo.green;if(!h.start&&h.teeCenter)h.start={...h.teeCenter,estimated:true};store.setCurrent(round);ensureMap();renderRound();frameHole();$('mapPrompt').textContent=h.start?'Choose a club, then tap where the shot finished.':'Tap the tee/ball position, then choose a club.';};
  ...
  mapTap=function(p){const h=round?.holes?.[round.currentHole],before=h?.shots?.length||0;oldMapTap(p);const after=h?.shots?.length||0;if(after>before){const shot=h.shots[after-1];shot.lie='checking lie…';store.setCurrent(round);renderRound();classifyLie(shot.end,h).then(lie=>{shot.lie=lie;store.setCurrent(round);renderRound();});setTimeout(frameHole,60);}};
})();